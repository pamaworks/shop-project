...
$(document).ready(function() {
	...
	$("#btn_delete_check").on("click", function() {
		...
		if (result) {
			...
			$("input[name='check']:checked").each(function(i) {
				var cart_code = $(this).val(); // 선택된 장바구니 코드값
				checkArr.push(cart_code); // 삭제하고자 하는 장바구니 코드를 배열형태로 저장
			});

			// 배열값을 ajax로 전송시 스프링에서 받는 파라미터  처리 구문
			$.ajax({
				url : '/cart/deleteChecked',
				type : 'post',
				dataType : 'text',
				data : {
					checkArr : checkArr // 키:값
				},
				success : function(data) {
					location.href = "/cart/list"; // 테이블의 장바구니 리스트를 삭제한 후, 장바구니 다시 요청 
				}
			});
		} else {
		}
	...
	$("button[name=btn_modify]").on(
			"click",
			function() {
				var cart_code = $(this).val(); // 장바구니 코드값 참조(숨겨놓기)
				var cart_amount = $( 
